...
export async function runWorkflow(params: {
  ...
  try {
    ...
    const insertStep = db.prepare(
      "INSERT INTO steps (id, run_id, step_id, agent_id, step_index, input_template, expects, status, max_retries, type, loop_config, on_fail_config, created_at, updated_at) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)"
    );
    ...
    for (let i = 0; i < workflow.steps.length; i++) {
      ...
      const onFailConfig = step.on_fail ? JSON.stringify(step.on_fail) : null;
      insertStep.run(stepUuid, runId, step.id, agentId, i, step.input, step.expects, status, maxRetries, stepType, loopConfig, onFailConfig, now, now);
    }
