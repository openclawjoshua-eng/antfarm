...
export function completeStep(stepId, output) {
    ...
    const stepCheck = db.prepare("SELECT status FROM steps WHERE id = ?").get(stepId);
...
function advancePipeline(runId) {
    ...
    const next = db.prepare("SELECT id, step_id, step_index FROM steps WHERE run_id = ? AND status = 'waiting' ORDER BY step_index ASC LIMIT 1").get(runId);
    ...
    if (next && incomplete) {
        const earlierIncomplete = db.prepare("SELECT id FROM steps WHERE run_id = ? AND status IN ('failed', 'pending', 'running') AND step_index < ? LIMIT 1").get(runId, next.step_index);
    ...
    if (next) {
        ...
        const nextStep = db.prepare("SELECT agent_id FROM steps WHERE id = ?").get(next.id);
        if (nextStep?.agent_id) {
            triggerAgentCron(nextStep.agent_id).catch(() => { });
        }
...
export async function failStep(stepId, error) {
    const db = getDb();
    const step = db.prepare("SELECT run_id, step_id, retry_count, max_retries, type, current_story_id, on_fail_config FROM steps WHERE id = ?").get(stepId);
    if (!step)
        throw new Error(`Step not found: ${stepId}`);
    ...
    if (newRetryCount > step.max_retries) {
        ...
        const onFailConfig = step.on_fail_config ? JSON.parse(step.on_fail_config) : null;
        if (onFailConfig?.retry_step) {
            // Find the target step to retry within this run
            const targetStep = db.prepare("SELECT id, step_id, retry_count FROM steps WHERE run_id = ? AND step_id = ? LIMIT 1").get(step.run_id, onFailConfig.retry_step);
            ...
            if (targetStep && targetStep.retry_count < crossMaxRetries) {
                ...
                db.prepare("UPDATE steps SET status = 'waiting', output = ?, retry_count = 0, updated_at = datetime('now') WHERE id = ?").run(error, stepId);
                // Also reset any steps between target and current back to waiting
                db.prepare("UPDATE steps SET status = 'waiting', output = NULL, retry_count = 0, updated_at = datetime('now') WHERE run_id = ? AND step_index > (SELECT step_index FROM steps WHERE id = ?) AND step_index < (SELECT step_index FROM steps WHERE id = ?) AND status = 'done'").run(step.run_id, targetStep.id, stepId);
                // Set target step to pending with incremented retry_count
                db.prepare("UPDATE steps SET status = 'pending', retry_count = ?, output = NULL, updated_at = datetime('now') WHERE id = ?").run(targetStep.retry_count + 1, targetStep.id);
